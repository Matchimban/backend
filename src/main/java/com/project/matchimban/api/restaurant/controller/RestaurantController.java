...
public class RestaurantController {
    ...
    @Operation(summary = "매장 상태 변경", description = "- 매장 상태를 변경합니다. 이때 매장 상태는 대문자가 아닌 소문자로만 받아옵니다." +
            ...
            "\n- 다음 메서드로 변경 가능한 매장 상태 : PUBLISHED, INVISIBLE, DELETED" +
            "\n  - PUBLISHED: 관리자가 매장을 확인하여 등록한 상태" +
            "\n  - INVISIBLE: 매장을 일시적으로 보이지 않도록 설정한 상태" +
            "\n  - DELETED: 매장을 삭제 처리하는 상태, 대신 DB에는 데이터가 남아있다." +
            "\n- UNAUTHORIZED 등 다른 상태를 입력받으면 62001 에러가 발생합니다.")
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "60009", description = "[삭제 상태 변경 한정] 해당 회원의 매장이 아님."),
